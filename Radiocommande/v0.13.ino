...
*/
  bool OnOffTelec = false;
  bool OnOffDrone = false;
  bool EtatTelec = false;
  bool EtatDrone = false;
...
void LCD_Init()
  ...
  lcd.home();       //Alignement 1ère ligne & 1ère colonne
  lcd.setCursor(1, 0);
  lcd.print("Radiocommande");
  lcd.setCursor(0, 1);
  lcd.print("0.13 by NutsGuy");
  delay(1500);
  lcd.clear();
  }
...
*/    
  ...
  data.pitch    = mapJoystickValues( analogRead(A6), 12, 544, 1021, true );
  data.roll     = mapJoystickValues( analogRead(A7), 34, 522, 1020, true );
  data.SWX1     = digitalRead(7);
  data.SWX2     = digitalRead(8);
